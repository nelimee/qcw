...
def supports_routine(routine) -> ty.Tuple[bool, ty.Optional[str]]:
    ...
    """
    # If qiskit has not been imported, it is impossible that the given instance
    # is of any qiskit type, so we can already return False.
    if "qiskit" not in sys.modules:
        return (
            False,
            "'qiskit' has not been imported yet, the given routine cannot be "
            "of any Qiskit type.",
        )

    # An instance is supported if its type is either QuantumCircuit or
    # Instruction.
    from qiskit.circuit import QuantumCircuit
    from qiskit.circuit.instruction import Instruction

    if not (isinstance(routine, Instruction) or isinstance(routine, QuantumCircuit)):
        return (
            False,
            f"The provided routine of type '{type(routine).__name__}' is "
            f"neither an instance of '{QuantumCircuit.__name__}' nor "
            f"'{Instruction.__name__}'.",
        )
    else:
        return True, None
